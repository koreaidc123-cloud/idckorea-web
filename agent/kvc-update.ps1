...
#
#  ※ 2026-08-18 이전에 설치한 PC 는 원격 재부팅을 못 받습니다.
#    이 파일을 한 번 실행해 두면, 다음부터는 프로그램이 스스로
#    최신을 확인하므로 두 번 다시 찾아오지 않으셔도 됩니다.
# ═══════════════════════════════════════════════════════════════
...
try {
  ...
  Write-Host '      인터넷 연결을 확인해 주세요.' -ForegroundColor Red
  Write-Host ''; Read-Host '  엔터를 누르면 닫힙니다'; exit 1
}
...
$bad = $null
if ([string]::IsNullOrWhiteSpace($new) -or $new.Length -lt 5000) { $bad = '파일이 너무 짧습니다 (받다가 끊김)' }
...
Write-Host '  ─────────────────────────────────────────' -ForegroundColor DarkGray
Write-Host ''
Write-Host '  끝났습니다. 이제 이 PC 는 스스로 최신을 확인합니다.' -ForegroundColor Cyan
