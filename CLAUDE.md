...
### ★★★ 0순위 — 랙PC 14대 프로그램 갱신 (2026-08-19 현장 작업 중단 지점)

**원격 재부팅이 안 되던 원인은 두 겹이었고, 첫째는 해결했습니다.**

| | 상태 |
|---|---|
| ① Supabase `beat` 함수가 옛날 것 | ✅ **완료** — `db/06-B-사라진PC-잡기.sql` 실행함 |
| ② 랙PC 안의 프로그램이 8/16 자 | ⬜ **남음** — 14대 전부 |

①은 GitHub 에 push 해도 안 바뀝니다. Supabase SQL Editor 에서 직접 돌려야 합니다.
(`db/03-command.sql` 은 옛 beat 가 들어 있어 **실행 금지**)
성공 확인 방법 — `pcs_unregistered` 에 "대장에서 사라진 PC" 설명이 붙은 줄이 생깁니다.
그 문구는 새 beat 함수만 씁니다.

②는 원격으로 닿을 길이 없습니다. 8/16 자 프로그램에는 명령을 받아 실행하는
코드 자체가 없어서, 서버가 뭘 답하든 로그만 남기고 끝냅니다.

**가장 쉬운 방법 — 파일 하나 덮어쓰기**

애니데스크로 접속해서 `kvc-agent.ps1` 을 `C:\KVC` 에 붙여넣고 덮어쓰면 끝입니다.
`.bat` 도 PowerShell 도 필요 없습니다. 예약 작업이 2분마다 그 파일을 그냥
실행하므로, 파일만 바뀌어 있으면 다음 실행부터 새 프로그램이 돕니다.

  · 보낼 파일 — 바탕화면 `★PC등록_USB_최신\갱신용-이것만복사\kvc-agent.ps1`
  · 넣을 곳   — 그 PC 의 `C:\KVC\` (덮어쓰기)
  · `pc.json` 은 절대 건드리지 않습니다 (품번·열쇠). 재등록 필요 없습니다.
  · 확인 — 파일 크기가 약 21KB 면 최신 (옛날 것은 13KB)

`프로그램갱신.bat` 도 같이 넣어 뒀지만 안 쓰셔도 됩니다. 검사·백업·시험까지
자동으로 하도록 만든 것인데, 파일 복사가 편하신 분께는 오히려 번거롭습니다.

**이번이 마지막 수작업입니다.**
`kvc-agent.ps1` 에 자동 갱신을 넣었습니다. 하루 한 번 사이트의 최신본과 비교해
다르면 스스로 바꿉니다. 잘못된 파일 하나로 랙PC 전체가 멈추면 되돌릴 수 없으므로
길이·표식·문법 세 가지를 확인하고, 하나라도 어긋나면 바꾸지 않습니다.
바꾸기 전 파일은 `C:\KVC\kvc-agent.bak` 로 남고, 갱신이 실패해도 신호는 계속 갑니다.

> **앞으로 `agent/kvc-agent.ps1` 을 고칠 때는 각별히 조심하십시오.**
> 이제 이 파일은 push 하면 랙PC 전체에 하루 안에 퍼집니다.

**USB 는 바탕화면 `★PC등록_USB_최신` 폴더가 정본입니다.** (2026-08-19 USB 교체 완료)
`PC등록` · `PC등록_새버전` 두 폴더는 8/16 자라 쓰면 안 됩니다.
이름이 비슷해 어느 것이 최신인지 알 수 없었던 것이 이번 사고의 발단이었습니다.
아직 이름을 안 바꿔 뒀으니, 헷갈리면 `_옛날_쓰지마세요` 를 붙이십시오.
(저장소 `agent/` 와 내용은 같되, `kvc-config.json` 에 등록 암호가 채워져 있습니다)

### 그 밖에 남겨둔 것

- **작업실 테스트 PC 2대가 아직 신호를 보냅니다** — 관리자 [미등록 PC] 에 계속 뜹니다.
  애니데스크 `178 546 9640` (2200G) · `802 463 064` (5700G).
  그 PC 에서 `schtasks /Delete /TN "KVC-Agent" /F` 후 `C:\KVC` 를 지우면 멈춥니다.
  랙 운영에는 지장이 없어 급하지 않습니다.
- **오늘 만든 것 중 실제로 눌러보지 못한 것** — 수동 주문 배정 · 고객 정보 수정 ·
  세금계산서 버튼 · 원격 재부팅. 코드 검사와 서버 응답으로만 확인했습니다.
  14대 갱신을 마치신 뒤 한 번씩 시험해 보셔야 합니다.
